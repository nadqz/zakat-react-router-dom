...
const HasilPenghasilan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await GetAllData('zakatPenghasilan');
  ...
  const handleEdit = async (id) => {
    try {
      ...
      if (confirmEdit) {
        ...
        // Tambahkan logika navigasi atau perubahan state sesuai kebutuhan
      }
    } catch (error) {
    ...
    <div className='container'>
      ...
      <div className='content-penghasilan'>
        ...
        <div className='divider'></div>
          <h2>Hasil Perhitungan Zakat Penghasilan</h2>
          <div className='divider'></div>
          ...
          {data.map((value, index) => (
            <div key={index} className='form-penghasilan'>
              <div className='data-penghasilan-kiri'>
                ...
                <p>Penghasilan Tambahan per Bulan:</p>
                <p>Hutang/Cicilan per Bulan:</p>
                <p>Harga Emas Saat Ini (1 Gram):</p>
                <p style={{ fontWeight: 'bold' }}>Total Penghasilan:</p>
                <p style={{ fontWeight: 'bold' }}>Besar Nisab Zakat Penghasilan:</p>
                <p style={{ fontWeight: 'bold' }}>Jumlah Zakat Penghasilan yang Harus Dibayarkan per Bulan:</p>
            </div>
            <div className='data-penghasilan-kanan'>
                <p>{value?.date?.toDate().toLocaleDateString()}</p>
                <p>{value?.penghasilanBulanan}</p>
                <p>{value?.penghasilanLainnya}</p>
                <p>{value?.hutangCicilan}</p>
                <p>{value?.hargaEmas}</p>
                <p style={{ fontWeight: 'bold' }}>{value?.tot}</p>
...
export default HasilPenghasilan;
